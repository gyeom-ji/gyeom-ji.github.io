...
1. <span style="color:#9fb584">**각각 독립된 메모리 영역(Code, Data, Stack, Heap 구조)과 PCB**</span>로 구성되어 있다.
  ...
  - 코드 영역과 데이터 영역은 정적 할당 영역으로 프로그램이 실행되는 동안 크기가 변하지 않는다.
  - Stack영역과 Heap 영역은 동적 할당 영역으로 프로그램이 실행되는 동안 크기가 변할 수 있다.
...
- 프로세스는 <span style="color:#9fb584">**커널이 제공하는 IPC를 이용해 프로세스간 통신**</span>을 할 수 있다.
  - 정보 공유, 성능 제고, 모듈화
- IPC 에는 2가지 기본 모델이 존재한다.
