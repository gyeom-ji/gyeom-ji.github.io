...
struct BinarySearchTree<T: Comparable> {
    ...
    private search(_ node: Node<T>?, _ data: T) -> Bool {
    ...
    private mutating func removeMin(_ node: Node<T>?) -> Node<T>? {
        ...
        node.left = deleteMin(node.left) // 삭제될 node의 자식과 부모를 연결시켜 최솟값 삭제
        return node
    }
    ...
    private mutating func remove(_ node: Node<T>?, _ data: T) -> Node<T>? {
        ...
        if data < node.data {
            ...
            node.right = deleteMin(target.right) // 대체할 노드를 트리에서 분리시키고 최종적으로 삭제되는 노드(target)의 오른쪽 자식노드의 레퍼런스를 리턴 -> node의 오른쪽 자식으로 만든다.
...
final class Node {
    ...
    init(_ data: Int, _ x: Int,  _ left: Node? = nil, _ right: Node? = nil){
        self.data = data
        self.left = left
        self.right = right
    }
...
struct BinarySearchTree{
    ...
    private preorder(_ node: Node?, _ arr: inout [Int]) -> [Int] {
        guard let node = node {return}
    ...
    private postorder(_ node: Node?, _ arr: inout [Int]) -> [Int] {
        guard let node = node {return}
...
func solution(_ nodeinfo:[[Int]]) -> [[Int]] {
    ...
    var nodeinfo : [[Int]] = nodeinfo.map {
        index += 1
        return $0 + [index]
    }.sorted(by: {$0[1] > $1[1]})

    for node in nodeinfo {
        bst.append(node[0], node[2])
    }
